...
async def finalizar_pedido(id_pedido: int, session: Session = Depends(sessao), usuario: Usuario = Depends(verificar_token)):
    pedido = session.query(Pedido).filter(Pedido.id==id_pedido).first()
    if not pedido: 
        raise HTTPException(status_code=400, detail="Pedido não encontrado!")
    if not usuario.perfilAdmin and usuario.id != pedido.usuario:
    ...
    session.commit()
    return {
        "mensagem": "Pedido #{} finalizado com sucesso!".format(pedido.id),
        "pedido": pedido
            }

@pedidos_router.get("/pedido/{id_pedido}")
async def visualizar_pedido(id_pedido: int, session: Session = Depends(sessao), usuario: Usuario = Depends(verificar_token)):
    pedido = session.query(Pedido).filter(Pedido.id==id_pedido).first()
    if not pedido: 
        raise HTTPException(status_code=400, detail="Pedido não encontrado!")
    if not usuario.perfilAdmin and usuario.id != pedido.usuario:
...
@pedidos_router.get("/listar/pedidos-usuario", response_model=List[ResponsePedidosSchema])
async def listar_pedidos(session: Session = Depends(sessao), usuario: Usuario = Depends(verificar_token)):
